...
{
    class ConditionGroupOnPercentage
    {
    ...
    class 特定品項的打折 : ADiscountStrategy
    {
        ...
        public override void Discount()
        {
            ...
            List<ConditionGroupOnPercentage> avaliableConditions = items.Select(x =>
            {
                ...
                if (condition == null)
                    return null;
                ConditionGroupOnPercentage avaliableCondition = new ConditionGroupOnPercentage()
                {
                    Name = x.Name,
            ...
            int reduceAmount = (int)(conditionsGroup.Select(x => x.SelectMany(y => Enumerable.Repeat(y.Price, y.BuyCount)).OrderBy(Price => Price).Take(freeCount* x.Key.ConditionCount).Sum()).Sum()* discountStrategy.Rewards[0].Percentage);
            ...
            //    }
            //}
            ////conditions ORDER BY DESC By Price
            //for(int i=0;i< contitionNames.Count; i++)
            //{
            //    for(int j=0;j< contitionNames[i].Count; j++)
            //    {
            ...
            //}          
            //reduceAmount= (int)(subTotalAmount * discountStrategy.Rewards[0].Percentage);
            //MealItem freeItem = new MealItem("(折)"+discountStrategy.Name, reduceAmount, 1);
            //items.Add(freeItem);
            #endregion
        }
